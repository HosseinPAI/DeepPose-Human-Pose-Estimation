...
def prepare_joints(dataset_path, train_data, valid_data, test_data, img_size):
    ...
    joints = torch.zeros((2000, 28))
    joints_2d = torch.zeros((2000, 2, 14))

    # for Train Joints
    for batch_idx, (images, label) in enumerate(train_data):
        for i in range(len(label[0])):
            img = io.imread(label[0][i])
            scaled_joint = update_joint_coordinate(joints_coord[int(label[0][i][-8:-4]) - 1], img, img_size,
                                                   mode='resize')
            ...
            for j in range(14):
                ...
                joints[int(label[0][i][-8:-4]) - 1][2 * j + 1] = scaled_joint[1][j]
...
def prepare_data(args, dataset_path, train_ratio, valid_ratio, test_ratio, image_size=220):
    ...
    data = MyImageFolder(os.path.join(dataset_path, ''), transform=data_transform)
    train_set, valid_set, test_set = random_split(data, [int(len(data) * train_ratio), int(len(data) * valid_ratio),
                                                         int(len(data) * test_ratio)])
    train_data_loader = DataLoader(train_set, batch_size=128, shuffle=True, **args)
    valid_data_loader = DataLoader(valid_set, batch_size=128, shuffle=True, **args)
    test_data_loader = DataLoader(test_set, batch_size=128, shuffle=True, **args)

    true_joints, true_joints_2d = prepare_joints(dataset_path, train_data_loader, valid_data_loader, test_data_loader,
                                                 (image_size, image_size))

    return train_data_loader, valid_data_loader, test_data_loader, true_joints, true_joints_2d


def train_data_augmentation(train_data, true_joints, true_joints_2d, batch_size):
    ...
    for batch_idx, (images, label) in enumerate(train_data):
        ...
        target = true_joints_2d[train_label[(batch_idx * batch_size):end_index]]

        # Start for Aug,entation
        batch_image = np.transpose(images[0].numpy(), (0, 2, 3, 1)).reshape(len(images[0]), 1, 220, 220, 3)
        ...
        for i in range(len(batch_image)):
            image_aug[i], kps_aug[i] = seq(images=batch_image[i], keypoints=joint[i])

        # Save Image Augmentation
        for i in range(len(image_aug)):
            train_data_aug.append(torch.tensor(np.transpose(np.float32(image_aug[i]), (2, 0, 1))))
        ...
        for i in range(len(kps_aug)):
            ...
            true_joints_2d_aug[train_label_aug[data_len + i + index]] = temp_joint
            temp_2d = coordinate_normalize(temp_joint)
            for j in range(14):
                true_joints_aug[train_label_aug[data_len + i + index]][2 * j] = temp_2d[0][j]
                true_joints_aug[train_label_aug[data_len + i + index]][2 * j + 1] = temp_2d[1][j]
